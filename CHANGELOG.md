...
### Fixed
- **Critical: "nonce too low" error when signing transactions via dApp browser**
  - Fixed `nonce` and `chainId` parsing in iOS (Swift) and Android (Kotlin) native modules
  - Now correctly handles values whether sent as Int, String (decimal), or String (hex with 0x prefix)
  - Root cause: ethers.js sends nonce/chainId as strings, but native code was casting directly to Int, defaulting to 0

### Changed
- iOS `ExpoTrustCoreModule.swift`: Robust parsing for nonce/chainId with hex support
- Android `ExpoTrustCoreModule.kt`: Robust parsing for nonce/chainId with hex support
